...
def make_trajectory(
    ...
    input_dim = generated_full[0].shape[1]

    # Pour affichage seulement : slicing en 2D
    generated_vis = [g[:, :2] for g in generated_full]
    ...
    # === Grille de score avec input_dim
    resolution = 50
    stride = 3
    arrow_scale = 0.5
    ...
    grid_points = torch.zeros(N_grid, input_dim, device=device)
    grid_points[:, 0] = X.reshape(-1)
    grid_points[:, 1] = Y.reshape(-1)
    ...
    with writer.saving(fig, save_path, dpi=200):
        for i in range(len(generated_vis)):
            t_val = torch.full((N_grid, 1), time[i], device=device)

            with torch.no_grad():
                score_vec = score_model(grid_points, t_val)  # [N, input_dim]

            # Affichage : uniquement les dimensions 0 et 1 du score
            U = score_vec[:, 0].cpu().numpy().reshape((resolution, resolution))
            V = score_vec[:, 1].cpu().numpy().reshape((resolution, resolution))
            mag = np.sqrt(U**2 + V**2) + 1e-8
            U_scaled = (U / mag) * arrow_scale
            ...
            # Pour fond : norme complète du score
            score_norm = score_vec.norm(dim=1).cpu().numpy()
            score_grid = score_norm.reshape((resolution, resolution))
            norm_image.set_data(score_grid)
            norm_image.set_clim(0, score_grid.max())
            ...
            quiver.set_UVC(U_scaled[::stride, ::stride], V_scaled[::stride, ::stride])
            gen_scat.set_offsets(generated_vis[i])

            if plot_traj:
                for line_idx, sample_idx in enumerate(idx_traj):
                    traj_x = [generated_vis[j][sample_idx, 0] for j in range(i + 1)]
